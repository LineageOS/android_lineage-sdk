...
public class LineageDatabaseHelper extends SQLiteOpenHelper{
    ...
    /**
     * Determine the path of the database, triggering any old db file rename tasks in the process.
     */
    private static String getDbPath(Context context, int userId) {
        ...
        if (!FileUtils.fileExists(dbPath)) {
            final String dbOldPath = dbNameForUser(context, userId, DATABASE_NAME_OLD);
            // Only rename databases that we know we can write to later.
            if (FileUtils.isFileWritable(dbOldPath)) {
                renameDb(dbOldPath, dbPath);
            }
        }
    ...
    public LineageDatabaseHelper(Context context, int userId) {
        super(context, getDbPath(context, userId), null, DATABASE_VERSION);
        mContext = context;
        mUserHandle = userId;
