...
public final class LineageButtons {
    ...
    // TODO: load this from settings, somehow.
    private boolean mVolBtnMusicControls = true;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_DISPATCH_VOLKEY_WITH_WAKELOCK:
                    ...
                    dispatchKeyToAudioService(ev);
                    dispatchKeyToAudioService(KeyEvent.changeAction(ev, KeyEvent.ACTION_UP));
                    break;
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_UP:
                if (down) {
                    mIsLongPress = false;
                    // queue skip event
                    int newKeyCode = (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN ? KeyEvent.KEYCODE_MEDIA_PREVIOUS
                    ...
                    if (DEBUG)
                        Slog.d(TAG, "Queueing media " + (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN ? "previous" : "next") + " event "
                                + newEvent);
                    Message msg = mHandler.obtainMessage(MSG_DISPATCH_VOLKEY_WITH_WAKELOCK, newEvent);
