/*
 * Copyright (c) 2019, The LineageOS Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package lineageos.health;

import android.annotation.NonNull;
import android.annotation.Nullable;
import android.os.Parcel;
import android.os.Parcelable;

import lineageos.os.Build;
import lineageos.os.Concierge;
import lineageos.os.Concierge.ParcelInfo;

import java.util.Date;
import java.util.Objects;

/**
 * The base Record class. Every data object stored in the
 * database must be a subclass of Record.
 */
public class Record implements Parcelable {
    /** @hide */
    public final static long UNSET_UID = -1L;
    public final static int UNSET_CATEGORY = -1;
    private final static Date UNSET_DATE = new Date(0L);

    protected final long uid;
    private final int category;

    protected Date time;
    protected Date end;

    /**
     * Constructor for instant records with an uid
     *
     * @see {@link lineageos.health.HealthStoreInterface#getRecords}
     * @see {@link lineageos.health.HealthStoreInterface#getRecord}
     * @see {@link lineageos.health.HealthStoreInterface#writeRecord}
     *
     * @param uid The unique id number of the record
     * @param time The time of when the record was made
     * @param category The category of the record
     */
    public Record(Date time, int category) {
        this(UNSET_UID, time, UNSET_DATE, category);
    }

    /**
     * Constructor for instant records with an uid
     *
     * @see {@link lineageos.health.HealthStoreInterface#getRecords}
     * @see {@link lineageos.health.HealthStoreInterface#getRecord}
     * @see {@link lineageos.health.HealthStoreInterface#writeRecord}
     *
     * @param uid The unique id number of the record
     * @param time The time of when the record was made
     * @param category The category of the record
     */
    public Record(long uid, Date time, int category) {
        this(uid, time, UNSET_DATE, category);
    }

    /**
     * Constructor for records with a time length and an uid
     *
     * @see {@link lineageos.health.HealthStoreInterface#getRecords}
     * @see {@link lineageos.health.HealthStoreInterface#getRecord}
     * @see {@link lineageos.health.HealthStoreInterface#writeRecord}
     *
     * @param uid The unique id number of the record
     * @param time The time of when the record was made
     * @param end The time of when the record ended
     * @param category The category of the record
     */
    public Record(long uid, Date time, Date end, int category) {
        this.uid = uid;
        this.time = time;
        this.end = end;
        this.category = category;
    }

    /** @hide */
    public Record(byte[] bytes) {
        this(fromBytes(bytes));
    }

    private Record(@NonNull Parcel parcel) {
        ParcelInfo parcelInfo = Concierge.receiveParcel(parcel);
        int parcelableVersion = parcelInfo.getParcelVersion();

        if (parcelableVersion >= Build.LINEAGE_VERSION_CODES.JACKFRUIT) {
            uid = parcel.readLong();
            time = new Date(parcel.readLong());
            end = new Date(parcel.readLong());
            category = parcel.readInt();
            readValueFrom(parcel);
        } else {
            uid = UNSET_UID;
            time = UNSET_DATE;
            end = UNSET_DATE;
            category = UNSET_CATEGORY;
        }

        // Complete parcel info for the concierge
        parcelInfo.complete();
    }

    /**
     * Write the value object to a {@link android.os.Parcel}
     *
     * {@link lineageos.health.Record#writeToParcel(Parcel, int)}
     *
     * @param dest The parcel on which to write the value
     */
    protected void writeValueTo(@NonNull Parcel dest) {
        // Do nothing
    }

    /**
     * Read the value object from a {@link android.os.Parcel}
     *
     * @see {@link lineageos.health.Record#Record(Parcel)}
     *
     * @param parcel The parcel from which the value will be read.
     */
    protected void readValueFrom(Parcel parcel) {
        // Do nothing
    }

    /**
     * Get symbol used for the measurement unit.
     * Will return <code>null</code> if no symbol is available
     */
    @Nullable
    public String getSymbol() {
        return null;
    }

    /** @hide */
    @Override
    public int describeContents() {
        return 0;
    }

    /** @hide */
    @Override
    public void writeToParcel(Parcel dest, int flags) {
        // Tell the concierge to prepare the parcel
        ParcelInfo parcelInfo = Concierge.prepareParcel(dest);

        // ==== JACKFRUIT ====
        dest.writeLong(uid);
        dest.writeLong(time.getTime());
        dest.writeLong(end.getTime());
        dest.writeInt(category);
        writeValueTo(dest);

        // Complete parcel info for the concierge
        parcelInfo.complete();
    }

    public final long getUid() {
        return uid;
    }

    public final Date getTime() {
        return time;
    }

    public final void setTime(Date time) {
        this.time = time;
    }

    public final Date getEnd() {
        return end;
    }

    public final void setEnd(Date end) {
        this.end = end;
    }

    /**
     * Get the category uid for this record.
     *
     * Reference values:
     * <b>System categories</b>
     * <ul>
     * <li>000 - 099: Hearth & Blood [API 10]</li>
     * <li>100 - 199: Body [API 10]</li>
     * <li>200 - 499: Breathing [API 10]</li>
     * <li>300 - 399: Sleep & Mindfulness [API 10]</li>
     * <li>400 - 499: Activity [API 10]</li>
     * </ul>
     */
    public final int getCategory() {
        return category;
    }

    public boolean validate() {
        return time.getTime() <= System.currentTimeMillis();
    }

    /** @hide */
    public byte[] asByteArray() {
        Parcel parcel = Parcel.obtain();
        writeToParcel(parcel, 0);
        byte[] bytes = parcel.marshall();
        parcel.recycle();
        return bytes;
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (!(o instanceof Record)) return false;
        Record that = (Record) o;
        return that.time.getTime() == time.getTime() &&
            that.end.getTime() == end.getTime() &&
            that.category == category;
    }

    @Override
    public int hashCode() {
        return Objects.hash(time, end, category);
    }

    @NonNull
    private static Parcel fromBytes(@NonNull byte[] bytes) {
        Parcel parcel = Parcel.obtain();
        parcel.unmarshall(bytes, 0, bytes.length);
        parcel.setDataPosition(0);
        return parcel;
    }

    @Override
    public String toString() {
        return String.format(
            "Record { uid: %1$d, time: %2$d; end: %3$d; category: %4$d }",
            uid, time.getTime(), end.getTime(), category
        );
    }

    /** @hide */
    public static final Parcelable.Creator<Record> CREATOR =
            new Parcelable.Creator<Record>() {

        @Override
        public Record createFromParcel(Parcel source) {
            return new Record(source);
        }

        @Override
        public Record[] newArray(int size) {
            return new Record[size];
        }
    };
}
