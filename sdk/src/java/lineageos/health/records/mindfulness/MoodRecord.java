...
public class MoodRecord extends Record {
    ...
    public MoodRecord(Date time, int moodLevel) {
        this(UNSET_UID, time, moodLevel, "");
    }

    public MoodRecord(long uid, Date time, int moodLevel) {
        this(uid, time, moodLevel, "");
    }

    public MoodRecord(Date time, int moodLevel, @NonNull String notes) {
        this(UNSET_UID, time, moodLevel, notes);
    }

    public MoodRecord(long uid, Date time, int value, @NonNull String notes) {
    ...
    public String toString() {
        return String.format(
            "Record { uid: %1$d, time: %2$d; moodLevel: %3$d; notes: %4$s }",
            uid, time.getTime(), moodLevel, notes
        );
    }
}
