    ...
    public TouchscreenGesture[] getTouchscreenGestures() {
        try {
            if (isSupportedAIDL(FEATURE_TOUCHSCREEN_GESTURES)) {
                ...
                vendor.lineage.touch.Gesture[] gestures = touchscreenGesture.getSupportedGestures();
                TouchscreenGesture[] tsGestures = new TouchscreenGesture[gestures.length];
                for (int i = 0; i < gestures.length; i++) {
                    tsGestures[i] = (TouchscreenGesture) gestures[i];
                }
                return tsGestures;
            }
        } catch (Exception e) {
