...
public final class LineageHardwareManager {
        ...
        try {
            if (isSupportedHIDL(FEATURE_DISPLAY_MODES)) {
                ...
                final DisplayMode[] modes =
                        HIDLHelper.fromHIDLModes(displayModes.getDisplayModes());
                if (modes == null) {
                    return null;
                }
    ...
    public DisplayMode getCurrentDisplayMode() {
        try {
            if (isSupportedHIDL(FEATURE_DISPLAY_MODES)) {
                IDisplayModes displayModes = (IDisplayModes) mHIDLMap.get(FEATURE_DISPLAY_MODES);
                return remapDisplayMode(
                        HIDLHelper.fromHIDLMode(displayModes.getCurrentDisplayMode()));
            } else if (checkService()) {
    ...
    public DisplayMode getDefaultDisplayMode() {
        try {
            if (isSupportedHIDL(FEATURE_DISPLAY_MODES)) {
                IDisplayModes displayModes = (IDisplayModes) mHIDLMap.get(FEATURE_DISPLAY_MODES);
                return remapDisplayMode(
                        HIDLHelper.fromHIDLMode(displayModes.getDefaultDisplayMode()));
            } else if (checkService()) {
