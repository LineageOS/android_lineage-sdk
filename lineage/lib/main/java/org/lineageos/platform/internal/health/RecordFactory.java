...
import lineageos.health.records.life.*;
...
final class RecordFactory {
     ...
     * Performance is way more important in this situation as
     * we are reading data from storage which is a slow operation
     * on its own, we don't want additional overhead.
     * It also helps protecting against unwanted data types.
     */
    static Record build(int category, @NonNull byte[] source) {
        switch (category) {
            ...
            case HearthRateRecord.CATEGORY:
                return new HearthRateRecord(source);
