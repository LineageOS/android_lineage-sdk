...
public class HealthStoreService extends LineageSystemService {
    ...
    private List<Record> getRecordsImpl(int category, long begin, long end,
            int callingUid) {
        if (!hasAccess(callingUid, category)) {
            return new ArrayList<>();
        }

        return mRepo.getByCategory(category, begin, end);
    }
    ...
    private final IBinder mService = new IHealthStore.Stub() {
        ...
        public List<Record> getRecordsInTimeFrame(int category,
                long begin, long end) {
            enforceReadPermission();
            return getRecordsImpl(category, begin, end, getCallingUid());
        }
