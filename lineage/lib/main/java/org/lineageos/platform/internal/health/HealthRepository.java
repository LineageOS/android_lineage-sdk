...
public class HealthRepository {
    ...
    private HealthRepository() {
        setupDataFolder();
        mDb = HealthDatabase.getInstance();
    }

    @NonNull
    public static HealthRepository getInstance() {
        if (sInstance == null) {
            sInstance = new HealthRepository();
        }
        return sInstance;
    }
    ...
    public boolean deleteAllInCategory(int category) {
        return mDb.deleteAllInCategory(category);
    }
    ...
    private boolean setupDataFolder() {
        final File dataFolder = HealthDatabase.DB_FOLDER;
        if (!dataFolder.exists()) {
            dataFolder.mkdirs();
        }

        return SELinux.restoreconRecursive(dataFolder);
    }
}
