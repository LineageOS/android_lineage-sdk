...
/**
 * A clone of SQLiteOpenHelper that writes database in a
 * location specified in the constructor and is never dependent
 * from a Context instance. Note that it does not implement all of the
 * methods of AOSP's SQLiteOpenHelper.
 *
 * AOSP documentation:
 *
 ...
 * @hide
 */
abstract class AltSQLiteOpenHelper {
    private static final String TAG = "AltSQLiteOpenHelper";
     ...
     *
     * @param dbFile of the database file, or null for an in-memory database
     * @param factory to use for creating cursor objects, or null for the default
     * @param version number of the database (starting at 1); if the database is older,
     *     {@link #onUpgrade} will be used to upgrade the database; if the database is
     *     newer, {@link #onDowngrade} will be used to downgrade the database
     */
    AltSQLiteOpenHelper(@Nullable File dbFile,
            @Nullable CursorFactory factory, int version) {
        this(dbFile, factory, version, null);
    }
    ...
    AltSQLiteOpenHelper(@Nullable File dbFile,
        ...
        this(dbFile, factory, version, 0, errorHandler);
    }
     ...
     */
    AltSQLiteOpenHelper(@Nullable File dbFile, int version,
            @NonNull SQLiteDatabase.OpenParams openParams) {
        this(dbFile, version, 0, openParams.toBuilder());
    }

    /**
     * Same as {@link #AltSQLiteOpenHelper(File, CursorFactory, int, DatabaseErrorHandler)}
     ...
     * given dbFile and version
     *
     * @param dbFile the mDbFile of the database file, null for a temporary in-memory database
     ...
     * @see #AltSQLiteOpenHelper(String, CursorFactory, int, DatabaseErrorHandler)
    ...
    AltSQLiteOpenHelper(@Nullable File dbFile,
        ...
        this(dbFile, version, minimumSupportedVersion,
    ...
    private AltSQLiteOpenHelper(@Nullable File dbFile, int version,
            int minimumSupportedVersion,
            @NonNull SQLiteDatabase.OpenParams.Builder openParamsBuilder) {
        if (openParamsBuilder == null) throw new IllegalArgumentException("openParamsBuilder must not be null");
    ...
    private SQLiteDatabase getDatabaseLocked(boolean writable) {
        ...
        try {
            ...
            } else if (mDbFile == null) {
                db = SQLiteDatabase.createInMemory(mOpenParamsBuilder.build());
            } else {
                SQLiteDatabase.OpenParams params = mOpenParamsBuilder.build();
                try {
                    db = SQLiteDatabase.openDatabase(mDbFile, params);
                    // Keep pre-O-MR1 behavior by resetting file permissions to 660
                    setFilePermissionsForDb(mDbFile.getPath());
                } catch (SQLException ex) {
                    if (writable) {
                        throw ex;
                    }
                    Log.e(TAG, "Couldn't open " + mDbFile.getAbsolutePath()
            ...
            if (version != mNewVersion) {
                if (db.isReadOnly()) {
                    throw new SQLiteException("Can't upgrade read-only database from version " +
                            db.getVersion() + " to " + mNewVersion + ": " + mDbFile.getAbsolutePath());
                }
                ...
                if (version > 0 && version < mMinimumSupportedVersion) {
                    ...
                    if (SQLiteDatabase.deleteDatabase(databaseFile)) {
                        ...
                        throw new IllegalStateException("Unable to delete obsolete database "
                                + mDbFile.getAbsolutePath() + " with version " + version);
                    }
                } else {
                    db.beginTransaction();
                    try {
            ...
            if (db.isReadOnly()) {
                Log.w(TAG, "Opened " + mDbFile.getAbsolutePath() + " in read-only mode");
            }
    ...
    static void setFilePermissionsForDb(String dbPath) {
     ...
     * @see #AltSQLiteOpenHelper(String, CursorFactory, int, int, DatabaseErrorHandler)
     * @hide
     */
