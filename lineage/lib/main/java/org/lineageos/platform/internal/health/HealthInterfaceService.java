...
public class HealthInterfaceService extends LineageSystemService {
    ...
    private static final String BATTERY_HEALTH_CHANNEL_ID = "LineageHealthChargingControl";
    private static final int BATTERY_HEALTH_NOTIFICATION_ID = 1000;
    ...
    private void postBatteryHealthNotification() {
        String title = mContext.getString(R.string.battery_health_notification_title);
        String message = String.format(mContext.getString(R.string.battery_health_notification_content),
            mConfigLimit, msToString(0)); // TODO use targetTime instead of 0

        // TODO .addAction(int icon, CharSequence title, PendingIntent intent)
        Notification.Builder notification = new Notification.Builder(mContext, BATTERY_HEALTH_CHANNEL_ID)
        ...
        mNotificationManager.notify(BATTERY_HEALTH_NOTIFICATION_ID, notification.build());
    }

    private void createNotificationChannelIfNeeded() {
        String id = BATTERY_HEALTH_CHANNEL_ID;
        NotificationChannel channel = mNotificationManager.getNotificationChannel(id);
        if (channel != null) {
            return;
        }

        String name = mContext.getString(R.string.battery_health_notification_channel);
    ...
    private void cancelBatteryHealthNotification() {
        mNotificationManager.cancel(BATTERY_HEALTH_NOTIFICATION_ID);
    }
