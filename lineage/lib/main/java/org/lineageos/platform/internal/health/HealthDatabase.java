...
class HealthDatabase extends SQLiteOpenHelper {
    ...
    private HealthDatabase(@NonNull Context context) {
        super(context, DB_NAME, null, DB_VERSION);
    }

    static HealthDatabase getInstance(@NonNull Context context) {
        if (sInstance == null) {
    ...
    boolean writeRecord(@Nullable Record record) {
        ...
        android.util.Log.e("OHAI", record.toString());
    ...
    List<Record> getByCategory(int category) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                Record r = RecordFactory.build(category, source);
                // TODO: remove the next line
                android.util.Log.e("OHAI", r.toString());
                list.add(r);
