...
class HealthDatabase extends AltSQLiteOpenHelper {
    ...
    // DB stored in credential-encrypted system data folder
    static final File DB_FOLDER = new File(
        Environment.getDataSystemCeDirectory(UserHandle.USER_SYSTEM),
        "health"
    );
    private static final File DB_FILE = new File(
        DB_FOLDER, "healthStore.db");
    ...
    boolean writeRecord(@Nullable Record record) {
        ...
        cv.put(RECORD_TIME, record.getTime().getTime());
    ...
    MedicalProfile getMedicalProfile() {
        ...
        if (cursor.moveToFirst()) {
            ...
            int biologicalSex = cursor.getInt(1);
            float height = cursor.getFloat(2);
            int bloodType = cursor.getInt(3);
            int organDonor = cursor.getInt(4);
            String allergies = cursor.getString(5);
            String medications = cursor.getString(6);
            String conditions = cursor.getString(7);
            String notes = cursor.getString(8);
            profile = new MedicalProfile(biologicalSex,
                height, bloodType, organDonor,
